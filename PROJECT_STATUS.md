...
- Logic self-check currently passes: `87/87`.
...
- `tools/sample_replay.py` now includes a safe `session_summary` that lists observed events, chosen events, dry-run outputs, and next validation steps without raw player/HUD/combat/award text.
...
- `tests/run_logic_tests.py` is the no-host logic self-check and should report `87/87 passed`.
