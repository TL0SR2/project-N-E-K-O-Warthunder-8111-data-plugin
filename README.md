...
- 2026-06-21 / 2026-06-23 真机 `dry_run` smoke 已通过：Hosted UI context/action、pause/resume 安全门、spawn、overspeed warning/critical、low_alt、stall、overheat、identity manual seam、owned kill/death ownership、you_died 决策链路、dry_run dispatcher 输出均正常。
...
- kill/hudmsg/combat.feed/awards 等自由文本真实播报仍需先完成真机 dry_run 验证；2026-06-23 已验证手动 identity 会反映到 `combat.self.source=manual`，并观察到 `is_my_kill=true` 与 `is_my_death=true` 的 owned combat.feed 路径。`you_died` 已 dry_run 输出；`you_killed` 候选曾被 `SPAWNING` 门控压住，已在 `0ae2ff4` 修复为出生期允许 owned combat kill，仍需下一轮真机复测 dry_run 输出。stall/low_alt/overheat/overspeed 等数值安全事件不被 T-Safety 阻塞；`low_fuel` 已确认燃油数值接缝存在，但事件未确认。
...
- 真机 dry_run smoke 已完成两轮；2026-06-23 已观察到 `overspeed_warn` / `overspeed_critical`、`low_alt_danger`、`stall_risk`、`overheat`、`you_died` 进入 Arbiter / Dispatcher dry_run，并验证手动 identity 与 owned combat.feed 归属字段。
...
优先顺序：
1. 继续 M3 剩余验证：`you_killed` 在 `SPAWNING` 放行修复后的真机复测、replay 真实样本验证、`low_fuel` 慢速单项验证、油温/动力故障字段策略；继续观察 T-Observe 是否足够解释链路。
2. 继续真机 checklist，补 replay、kill/awards/free-text dry_run 接缝；identity/ownership 已有真机正向证据，但 `you_killed` dry_run 输出还需复测确认不再被 `scenario_gated(SPAWNING)` 压住。
...
- `/api/identity` 是 player_name 的主路径；插件侧 Hosted UI/context/action 接缝已完成，2026-06-23 真机已验证手动身份会反映到 `combat.self.source=manual`，并能驱动 `is_my_kill` / `is_my_death` owned combat.feed 标记；`you_killed` 的 post-fix dry_run 输出仍需复测。
